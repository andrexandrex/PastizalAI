...
def extract_connected_regions(mask_1d, label_value):
    """
    Extracts connected regions (components) of a given label_value from a mask.
    
    Returns a list of dictionaries, each containing:
      - 'bbox': (min_row, min_col, max_row, max_col)
      - 'binary_mask': full-size binary mask for that region.
    """
    label_mask = (mask_1d == label_value).cpu().numpy().astype(np.uint8)
    labeled = cc_label(label_mask, connectivity=2)  # 8-connected
    regions_info = []
