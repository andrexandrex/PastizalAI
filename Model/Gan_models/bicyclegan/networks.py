...
def get_non_linearity(layer_type='lrelu'):
    if layer_type == 'relu':
...
def define_G(input_nc, output_nc, nz, ngf, netG='unet_128', norm='batch', nl='relu',
             use_dropout=True, init_type='xavier', init_gain=0.02, gpu_ids=[], where_add='input', upsample='bilinear'):
    net = None
...
def define_D(input_nc, ndf, netD, norm='batch', nl='lrelu', init_type='xavier', init_gain=0.02, num_Ds=1, gpu_ids=[]):
    ...
    if netD == 'basic_128':
        ...
        net = D_NLayersMulti(input_nc=input_nc, ndf=ndf, n_layers=4, norm_layer=norm_layer, num_D=num_Ds)
    else:
...
########################discriminator layer adding noise#####################
...
class D_NLayersMulti(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3,
        ...
        # st()
        self.num_D = num_D
        self.gaussian_noise = GaussianNoise(std=0.1, decay_rate=0.002)   # Or pass std as argument
        if num_D == 1:
    ...
    def forward(self, input):
        input = self.gaussian_noise(input)
        if self.num_D == 1:
...
class GANLoss(nn.Module):
    ...
    def __call__(self, predictions, target_is_real, custom_label=None):
        ...
        for i, prediction in enumerate(predictions):
            if self.gan_mode in ['lsgan', 'vanilla']:
                if custom_label is not None:
                    # If user passes a list of labels for multi-scale D
                    if isinstance(custom_label, list):
                        label_i = custom_label[i]
                    else:
                        label_i = custom_label
                    loss = self.loss(prediction, label_i)
                else:
                    # Use the built-in real/fake label logic
                    target_tensor = self.get_target_tensor(prediction, target_is_real)
                    loss = self.loss(prediction, target_tensor)

            elif self.gan_mode == 'wgangp':
                # WGAN-GP doesn't use "labels" the same way
                # Typically:
                if custom_label is not None:
                    raise NotImplementedError("WGAN-GP + custom_label not implemented.")
                if target_is_real:
...
class G_Unet_add_all(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=True, upsample='basic'):
...
class E_NLayers(nn.Module):
    def __init__(self, input_nc, output_nc=1, ndf=64, n_layers=3,
                 norm_layer=None, nl_layer=None, vaeLike=True):
