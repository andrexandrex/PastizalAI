...
from logger_utils import CSVWriter, write_dict_to_json
from dataset_peru_train import get_dataloaders_for_training
...
def batch_train(FLAGS,mode:str):
    ...
    suffix = mode
    local_dir = os.path.join('/tmp', 'models', FLAGS.which_model,suffix)
    if not os.path.isdir(local_dir):
        os.makedirs(local_dir)
        print(f"Created local directory: {local_dir}")
    # Directory on Google Drive
    
    dir_path = os.path.join(FLAGS.dir_model, FLAGS.which_model,suffix)
    dir_state = os.path.join(FLAGS.dir_state, FLAGS.which_model,suffix)
    ...
    last_epoch, last_state = _find_last_state(original_state_dir, FLAGS.which_model)
    ...
    if FLAGS.continue_training and last_state is not None:
        ...
        if mode == 'freeze_backbone_baseline':
            ...
            # full_finetune: only load weights, start fresh optimizer
            print("→ Loaded weights from epoch", last_epoch, "but starting fresh optimizer for full_finetune")
            start_epoch = last_epoch + 1
            append_metrics = False  # or True, depending if you want to append to metrics
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        t_1 = time.time()
        train_loss = train_loop(train_dataset_loader, oil_spill_seg_model, loss_fn, optimizer, device)
